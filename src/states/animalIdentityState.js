...
async function createAnimalIdentity (context, payload) {
    ...
    if(info1.getLegalid() != info.getLegalid()) {
        ...
        context.addEvent('CattleChain/CreateIdentity', [['address',address], ['asset_id', info.getLegalid()],['status', 'true'],['message', 'asset created']], 'asset created');
    } else {
        console.log('asset already exits');
        context.addEvent('CattleChain/CreateIdentity', [['address',address], ['asset_id', info.getLegalid()],['status', 'false'], ['message', 'asset already exist']], 'asset already exist');       
    }
}
...
async function addAnimalEvents(context, payload) {
    ...
    if (info.getLegalid() == null || info.getLegalid() == '') {
        ...
        context.addEvent('CattleChain/AddAnimalEvent', [['status', 'false'],['message', 'legal ID is missing']], 'legal Id is missing');
    } else {
    var address = createAccount(info.getLegalid());
    const data = await context.getState([address]);
    const info1 = animalIdentity_pb.AnimalIdentity.deserializeBinary(data[address]);
    //asset exists
        if (info1.getLegalid() == info.getLegalid()) {
            ...
            for (const item of info.getEventsList()) {
                ...
                context.addEvent('CattleChain/AddAnimalEvent', [['address',address], ['asset_id', info.getLegalid()], ['payload', payload.toString()], ['status', 'true'], ['message', 'asset does not exist']], 'asset does not exist');       
            }
        } else {
            console.log('asset does not exists');
            context.addEvent('CattleChain/AddAnimalEvent', [['address',address], ['asset_id', info.getLegalid()],['status', 'false'], ['message', 'asset does not exist']], 'asset does not exist');       
        }
    }
}
